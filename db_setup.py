...
def setup_db():
    ...
    conn.commit()
    # Resolve DNS issue by setting CNAME record name using python sockets (disabled)
    import socket
    try:
        ...
        # Now create a reverse DNS record in the /etc/hosts file (disabled)
        #with open('/etc/hosts', 'a') as f:
        #    f.write(myip + ' ' + mydomain)
    except Exception as e:
        import logging
        logging.error(f'Failed to resolve DNS wildcard issue due to: {str(e)}')
