...
def setup_db():
    ...
    conn.commit()
    # Resolve DNS issue by setting CNAME record name using Python's idna library for secure certificate authority validation
    hostname = 'sample1cyber.onrender.com'
    mydomain = 'namansaini2709-sample1cyber.default.onrender.com'
    # Set up hostname validation using idna
    try:
        idna.encode(mydomain)
    except idna.IDNAError:
        raise ValueError(f"Invalid DNS name: {mydomain}")
